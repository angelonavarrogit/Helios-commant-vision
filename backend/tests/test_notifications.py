...
def _email(db_session: Session, account: EmailAccount, mid: str) -> Email:
    row = Email(account_id=account.id, provider_message_id=mid, content_hash=mid * 8)
    db_session.add(row)
    db_session.flush()
    return row
...
def test_query_summary_counts(db_session: Session, account: EmailAccount) -> None:
    e1 = _email(db_session, account, "q1")
    e2 = _email(db_session, account, "q2")
...
def test_query_urgent_lists_high(db_session: Session, account: EmailAccount) -> None:
    e1 = _email(db_session, account, "q3")
...
def test_query_pending(db_session: Session, account: EmailAccount) -> None:
    e1 = _email(db_session, account, "q4")
